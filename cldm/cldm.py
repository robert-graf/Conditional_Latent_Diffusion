...
def ControlLDM_from_LDM(ldm_config_path: dict,ldm_ckpt_path, *args, **kwargs):
    ...
    model= ControlLDM(*args, unet_config=unet_config, first_stage_config=first_stage_config, cond_stage_config=cond_stage_config,ckpt_path=ldm_ckpt_path, **config, **kwargs)
    return model
...
class ControlLDM(LatentDiffusion):
    ...
    def get_input(self, batch, _k, bs=None, *args, **kwargs):
        out = super().get_input(batch, self.first_stage_key,bs=bs, *args, **kwargs)
        x, c = out[:2]
    ...
    @torch.no_grad()
    def sample_log(self, cond, batch_size, ddim, ddim_steps, **kwargs):  # noqa: ARG002
        if ddim:
