...
from taming.modules.losses.vqperceptual import LPIPS, NLayerDiscriminator, adopt_weight, hinge_d_loss, vanilla_d_loss, weights_init
...
class LPIPSWithDiscriminator(nn.Module):
    ...
    def forward(self, inputs, reconstructions, posteriors, optimizer_idx, global_step, last_layer=None, cond=None, split="train", weights=None):
        ...
        if optimizer_idx == 1:
            ...
            log = {f"{split}/disc_loss": d_loss.clone().detach().mean(), f"{split}/logits_real": logits_real.detach().mean(), f"{split}/logits_fake": logits_fake.detach().mean()}
            return d_loss, log
