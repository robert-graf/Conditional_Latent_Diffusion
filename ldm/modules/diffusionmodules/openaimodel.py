...
from ldm.modules.diffusionmodules.util import avg_pool_nd, checkpoint, conv_nd, linear, normalization, timestep_embedding, zero_module


# dummy replace
def convert_module_to_f16(_x):
    pass


def convert_module_to_f32(_x):
    pass
...
class EncoderUNetModel(nn.Module):
    ...
    def forward(self, x, timesteps):
        ...
        if self.pool.startswith("spatial"):
            results.append(h.type(x.dtype).mean(dim=(2, 3)))
            h = th.cat(results, axis=-1)  # type: ignore
            return self.out(h)
        else:
            h = h.type(x.dtype)
            return self.out(h)
