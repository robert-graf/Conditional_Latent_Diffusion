...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        # timestep embedding
        temb = None
        assert not isinstance(x, dict), x.keys()
        # downsampling
        hs = [self.conv_in(x)]
        for i_level in range(self.num_resolutions):
            for i_block in range(self.num_res_blocks):
...
class SimpleDecoder(nn.Module):
    def __init__(self, in_channels, out_channels, *_args, **_kwargs):
        super().__init__()
...
class UpsampleDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for k, i_level in enumerate(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks + 1):
                h = self.res_blocks[i_level][i_block](h, None)  # type: ignore
            if i_level != self.num_resolutions - 1:
...
class FirstStagePostProcessor(nn.Module):
    def __init__(self, ch_mult: list, in_channels, pretrained_model: nn.Module | None = None, reshape=False, n_channels=None, dropout=0.0, pretrained_config=None):
        super().__init__()
    ...
    def forward(self, x):
        ...
        for submodel, downmodel in zip(self.model, self.downsampler, strict=False):
