...
def patches_from_image(img, p_size=512, p_overlap=64, p_max=800):
    ...
    if w > p_max and h > p_max:
        ...
        for i in w1:
            for j in h1:
                patches.append(img[i : i + p_size, j : j + p_size, :])  # noqa: PERF401
    else:
        patches.append(img)
...
def img_save(imgs, img_path):
    ...
    for i, img in enumerate(imgs):
        if img.ndim == 3:
            img = img[:, :, [2, 1, 0]]  # noqa: PLW2901
        new_path = os.path.join(os.path.dirname(img_path), img_name + str(f"_s{i:04d}") + ".png")
...
def split_imageset(original_dataroot, taget_dataroot, n_channels=3, p_size=800, p_overlap=96, p_max=1000):
    ...
    for img_path in paths:
        ...
        img_save(patches, os.path.join(taget_dataroot, os.path.basename(img_path)))
...
def calculate_ssim(img1, img2, border=0):
    ...
    if img1.ndim == 2:
        ...
        if img1.shape[2] == 3:
            ssims = []
            for i in range(3):
                ssims.append(ssim(img1[:, :, i], img2[:, :, i]))  # noqa: PERF401
...
def calculate_weights_indices(in_length, out_length, scale, kernel, kernel_width, antialiasing):  # noqa: ARG001
